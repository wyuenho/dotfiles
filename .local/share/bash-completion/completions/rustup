_rustup() {
    ...
    case "${cmd}" in
        rustup)
            opts="-v -q -h -V --verbose --quiet --help --version [+toolchain] install uninstall dump-testament show update check default toolchain target component override run which doc man self set completions help"
        ...
        rustup__dump__testament)
            ...
            ;;
        rustup__help)
            opts="install uninstall dump-testament show update check default toolchain target component override run which doc man self set completions help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
